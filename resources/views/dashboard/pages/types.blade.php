        ...
        <div class="row g-3">
            <div class="col-auto">
                <div class="position-relative">
                    <input class="form-control px-5" type="search" placeholder="Search Types">
            ...
            <div class="col-auto">
                <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    ...
                    <a href="{{ route('type.create') }}" class="btn btn-outline-primary px-4">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            @if($type->icon)
                                            <img src="{{ asset('storage/'.$type->icon) }}"
                                                alt="Icon"
                                            ...
                                            <div class="type-info">
                                                ...
                                                <p class="mb-0 text-secondary">{{ Str::limit($type->description, 40) }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        @if($type->status === \App\Enums\BooleanEnum::TRUE)
                                    ...
                                    <td>
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('type.edit', $type->id) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                            ...
                                            <form action="{{ route('type.toggle-status', $type->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')

                                                <button type="submit"
                                                    class="btn btn-sm btn-outline-{{ $type->status === \App\Enums\BooleanEnum::TRUE ? 'warning' : 'success' }}"
                                                    title="{{ $type->status === \App\Enums\BooleanEnum::TRUE ? 'Deactivate' : 'Activate' }}">
                                            ...
                                            <a href="{{ route('type.show', $type->id) }}" class="btn btn-sm btn-outline-secondary" title="View">
                                            ...
                                            <form action="{{ route('type.destroy', $type->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this type?')">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination round-pagination">
                                ...
                                @for($i = 1; $i <= $types->lastPage(); $i++)
                                    ...
                                    @endfor
                                    <li class="page-item {{ ($types->currentPage() == $types->lastPage()) ? 'disabled' : '' }}">
