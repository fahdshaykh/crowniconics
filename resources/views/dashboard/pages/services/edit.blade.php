...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // This value comes from backend (old input or existing service type_id)
        const selectedTypeId = "{{ old('type_id', $service->type_id ?? '') }}";

        // Function to load types
        function loadTypes(categoryId, preselected = null) {
                ...
                fetch(url)
                    ...
                    .then(data => {
                        data.forEach(type => {
                            ...
                            // preselect option if it matches
                            if (preselected && preselected == type.id) {
        ...
        // On edit: if category already selected, load types immediately
        if (categorySelect.value) {
            loadTypes(categorySelect.value, selectedTypeId);
        }
    });
</script>
